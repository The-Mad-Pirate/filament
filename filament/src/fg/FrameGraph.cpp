...
struct RenderTarget {
    ...
    void create(FrameGraph& fg, DriverApi& driver, TargetFlags const& targetFlags) noexcept {
        ...
        FrameGraphPassResources::RenderTargetInfo& renderTargetInfo = targetInfo;
        ...
        if (!imported) {
            ...
            if (importedRenderTarget) {
                ...
                renderTargetInfo.target = importedRenderTarget->targetInfo.target;
                width = desc.width;
                height = desc.height;
            } else if (attachments) {
                renderTargetInfo.target = driver.createRenderTarget(TargetBufferFlags(attachments),
                        width, height, desc.samples, colorFormat,
                        { color }, { depth }, {});
            }
        }
        renderTargetInfo.params = {};
        renderTargetInfo.params.clear        = targetFlags.clear;
        renderTargetInfo.params.discardStart = targetFlags.discardStart;
        renderTargetInfo.params.discardEnd   = targetFlags.discardEnd;
        renderTargetInfo.params.dependencies = targetFlags.dependencies;
        renderTargetInfo.params.left = 0;
        renderTargetInfo.params.bottom = 0;
        renderTargetInfo.params.width = width;
        renderTargetInfo.params.height = height;
    }
...
void Resource::create(DriverApi& driver) noexcept {
    // some sanity check
    if (!imported) {
        // FIXME: don't create the texture handle if this resource is never read
